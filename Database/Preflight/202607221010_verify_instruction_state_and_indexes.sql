...
-- Purpose: Measure data readiness before state, tenant, conversation, and index migrations.
-- This script deliberately performs no INSERT, UPDATE, DELETE, DDL, or transaction control.

-- 1. Completion state values to normalize before making completed NOT NULL.
SELECT completed, count(*) AS record_count
FROM digital.instructions
GROUP BY completed
ORDER BY completed NULLS FIRST;

-- 2. Notification state values to normalize before constraining them to 0 or 1.
...
-- 3. Client-facing support types that lack a tenant. Expected: zero rows before
-- adding the support-type tenant constraint.
...
-- 4. Replies whose root has a different tenant. Expected: zero rows.
...
-- 5. Replies that target another reply instead of a canonical root. Expected: zero rows.
SELECT reply.id AS reply_id,
...
-- 6. Existing indexes on digital.instructions. Use this with EXPLAIN (ANALYZE, BUFFERS)
-- in a sanitized environment before approving candidate tenant-aware indexes.
